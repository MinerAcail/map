...
//Add error checking
    if (global.program < 0) {
        print("Shader program compilation failed\n", .{});
        return;
    }
    global.aspect = aspect;
    ...
    const map_data_f32: []const f32 = @alignCast(std.mem.bytesAsSlice(f32, global.map_data.items));
...
pub export fn render() void {
    const map_data_f32: []const f32 = @alignCast(std.mem.bytesAsSlice(f32, global.map_data.items));
     ...
     // Defensive checks
    if (map_data_f32.len == 0) {
        print("No map data available for rendering\n", .{});
        return;
    }

    if (global.vao < 0 or global.program < 0) {
        print("Invalid VAO or Program for rendering\n", .{});
        return;
    }
    ...
    glUniform1f(global.aspect_loc, global.aspect);
    {
        ...
        const vertexCount = map_data_f32.len / 2;
         // Prevent potential out-of-bounds rendering
        if (vertexCount > 0) {
